...
local function get_proxy_port(ssl, mock)
  if ssl  == nil then ssl  = false end
  if mock == nil then mock = false end
  for _, server in ipairs(conf.proxy_servers) do
    if server.mock == mock then
  ...
  error("No proxy port found for ssl=" .. tostring(ssl) .. ", mock=" .. tostring(mock), 2)
end

--- Returns the proxy ip.
-- @param ssl (boolean) if `true` returns the ssl ip address
local function get_proxy_ip(ssl, mock)
  if ssl  == nil then ssl  = false end
  if mock == nil then mock = false end
  for _, server in ipairs(conf.proxy_servers) do
    if server.mock == mock then
  ...
  error("No proxy ip found for ssl=" .. tostring(ssl) .. ", mock=" .. tostring(mock), 2)
end

--- returns a pre-configured `http_client` for the Kong proxy port.
-- @name proxy_client
local function proxy_client(timeout, mock)
  local proxy_ip = get_proxy_ip(false, mock)
  local proxy_port = get_proxy_port(false, mock)
...
local function proxy_ssl_client(timeout, mock)
  local proxy_ip = get_proxy_ip(true, mock)
  local proxy_port = get_proxy_port(true, mock)
